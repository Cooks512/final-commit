...
def _build_candidates(profile):
    """Build candidate dict, adapting to dataset characteristics."""
    ...
    candidates = {
        ...
        # FIX: BayesianRidge now gets scaling like Ridge does
        "BayesianRidge": Pipeline([
            ("scaler", StandardScaler()),
            ("br", BayesianRidge())
        ]),
        # NEW: ElasticNet for feature selection under collinearity
        "ElasticNet": Pipeline([
            ("scaler", StandardScaler()),
            ("en", ElasticNetCV(
    ...
    if profile.n_train >= 80:
        candidates["LightGBM"] = lgb.LGBMRegressor(
            ...
            reg_lambda=2.0,         # more regularisation
